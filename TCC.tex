...
Depois, será escolhido 10 sites de notícias, seguindo um critérios específico.
Assim, será feito uma avaliação manual dos 10 sites para averiguar a acessibilidade de cada um deles.
Após isso, será feito uma compilação dos resultados, a partir da análise feita.
Tudo isso para fazer um comparativo da computação do grau de acessibilidade de cada um dos sites.
Por fim, será tirado as conclusões da pesquisa e do comparativo feito. Então, a metodologia, que será aplicada para que os objetivos sejam alcançados, será mostrado na figura 1.\\
...
\begin{figure}[!ht]
	...
	\begin{tikzpicture}[node distance = 3cm, auto]
		...
		\node [block, below of = sites]   (avaliacao) {Realizar uma avaliação manual dos sites};
		\node [block, right of = avaliacao, xshift=2cm]      (coletar) {Coletar dados};
		\node [block, up of = coletar, xshift=3.5cm, yshift=-2cm]   (comparativo)  {Fazer comparativo e análise dos sites};
		\node [block, up of = comparativo, xshift=3.5cm, yshift=0cm]     (conclusao) {Apresentar a conclusão};
		% Draw edges
